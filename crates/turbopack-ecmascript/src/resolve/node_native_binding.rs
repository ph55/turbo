...
use crate::references::raw::SourceAssetReference;
            ...
            if let FileContent::Content(ref config_file) = &*file.await? {
                ...
                for version in node_pre_gyp_config.binary.napi_versions.iter() {
                        ...
                        .iter()
                    {
                        ...
                        {
                            assets.insert(Vc::upcast(SourceAsset::new(*dylib)));
                        }
                    }
                    assets.insert(Vc::upcast(SourceAsset::new(resolved_file_vc)));
                }
                    ...
                    match entry {
                        ...
                        DirectoryEntry::Symlink(dylib) => {
                            ...
                            for symlink in realpath_with_links.symlinks.iter() {
                                assets.insert(Vc::upcast(SourceAsset::new(*symlink)));
                            }
                            assets.insert(Vc::upcast(SourceAsset::new(*dylib)));
                        }
                        _ => {}
