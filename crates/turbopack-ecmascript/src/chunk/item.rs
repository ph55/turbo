...
use turbopack_core::{
    ...
    chunk::{
        ...
        ChunkableAsset, ChunkingContext, FromChunkableAsset, ModuleId,
    },
...
impl FromChunkableAsset for Box<dyn EcmascriptChunkItem> {
    async fn from_asset(
        context: Vc<Box<dyn ChunkingContext>>,
        asset: Vc<Box<dyn Asset>>,
    ) -> Result<Option<Vc<Self>>> {
        let Some(placeable) = Vc::try_resolve_sidecast::<Box<dyn EcmascriptChunkPlaceable>>(asset).await? else {
            return Ok(None);
        };

        let Some(context) = Vc::try_resolve_sidecast::<Box<dyn EcmascriptChunkingContext>>(context).await? else {
            return Ok(None);
        };
