...
use turbopack_core::{
    asset::Asset,
    chunk::{
        availability_info::AvailabilityInfo, ChunkableAsset, ChunkingContext, FromChunkableAsset,
        ModuleId,
    },
...
impl PatternMapping {
    ...
    pub async fn resolve_request(
        ...
        if let Some(chunkable) = Vc::try_resolve_sidecast::<Box<dyn ChunkableAsset>>(asset).await? {
