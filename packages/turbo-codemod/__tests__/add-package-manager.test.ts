...
import * as utils from "@turbo/utils";
import * as workspaces from "@turbo/workspaces";
...
describe("add-package-manager", () => {
  ...
  test("no package manager - basic", async () => {
    ...
    const mockGetAvailablePackageManagers = jest
      .spyOn(utils, "getAvailablePackageManagers")
      .mockResolvedValue({
    ...
    const mockGetWorkspaceDetails = jest
      .spyOn(workspaces, "getWorkspaceDetails")
      .mockResolvedValue(
        getWorkspaceDetailsMockReturnValue({ root, packageManager })
      );
